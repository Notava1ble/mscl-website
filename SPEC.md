...
The organizing team's app will communicate with the website exclusively through HTTP endpoints exposed via Convex HTTP Actions. All endpoints must be secured with an `x-api-key` header.

### 4.1 Ingest / Update Match

**Endpoint:** `POST https://<your-convex-url>.convex.site/api/write/match`
...
**Endpoint:** `POST https://<your-convex-url>.convex.site/api/write/weeks/transition`
...
4. **New Player Onboarding:** Handled seamlessly during the End of Week Transition. If an unrecognized `name` is present in the `players` array payload, they are seamlessly inserted into the database.
